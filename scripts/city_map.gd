...
func _process(_delta: float) -> void:
    ...
    if player != null:
        _update_player_pin(player.global_position)
    _update_enemy_pins()
...
func _build_circular_surface(surface: Control) -> void:
    ...
    circle.modulate = Color(0.018,0.055,0.09,0.98)
    ...
    player_pin.modulate = Color(0.8,0.98,1.0)
...
func _update_enemy_pins() -> void:
    if map_surface == null:
        return
    for child in map_surface.get_children():
        if child.name.begins_with("EnemyPin"):
            child.queue_free()
    for enemy in get_tree().get_nodes_in_group("enemies"):
        if enemy is Node3D and is_instance_valid(enemy):
            ...
            pin.position = _world_to_map(enemy.global_position)-Vector2(8,8)
            pin.add_theme_font_size_override("font_size",10)
            pin.modulate = Color(1.0,0.3,0.25)
            pin.mouse_filter = Control.MOUSE_FILTER_IGNORE
            map_surface.add_child(pin)

func _world_to_map(world: Vector3) -> Vector2:
    return Vector2(clamp((world.x/WORLD_HALF+1.0)*0.5*MAP_SIZE,8.0,MAP_SIZE-8.0),clamp((world.z/WORLD_HALF+1.0)*0.5*MAP_SIZE,8.0,MAP_SIZE-8.0))

func _update_player_pin(world_position: Vector3) -> void:
    var point := _world_to_map(world_position)
    if player_pin != null: player_pin.position = point-Vector2(12,12)
    if player_ring != null: player_ring.position = point-Vector2(15,15)
    if full_player_pin != null: full_player_pin.position = _world_to_full_map(world_position)-Vector2(12,12)
...
func _toggle_full_map() -> void:
    if full_map == null: _create_full_map()
...
func _create_full_map() -> void:
    ...
    for point in POINTS: _place_full_point(point["pos"],point["label"],point["name"])
...
func _world_to_full_map(world: Vector3) -> Vector2:
    ...
    return Vector2(clamp((world.x/WORLD_HALF+1.0)*0.5*size.x,12.0,size.x-12.0),clamp((world.z/WORLD_HALF+1.0)*0.5*size.y,12.0,size.y-12.0))
